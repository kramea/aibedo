...
variables_in = ["SWTNT","SWTNTCLR","LWTUP","LWTUPCLR","LWGNT","SWGNT","LWGNTCLR","SWGNTCLR","HFLUX","EFLUX",
                "CLOUD","QI","QL"]
variables_out = ['cres','crel','cresSurf','crelSurf','netTOAcs','netSurfcs','clivi','clwvi','lcloud']
out_path = '/home/jupyter-haruki/work/MERRA2/MERRA2_Input_Exp8.nc'
run_preprocess(variables_in,variables_out,out_path)
...
sftlf,b2 = xr.broadcast(d , refdat.isel(time=range(492))['lcloud'], exclude=('lat','lon'))

lsMask = xr.DataArray(name='lsMask', data=sftlf.load(), attrs={'long_name':'land_sea_mask', 'Description':'land fraction of each grid cell 0 for ocean and 1 for land'}, coords={'time': refdat.isel(time=range(492))['time'],'lat': data['FRLAND'].lat,'lon': data['FRLAND'].lon})
output = lsMask.to_dataset(name = 'lsMask')
del indata,data
output.to_netcdf(path = out_path,mode='a')

## change units for variables
data = xr.open_dataset('/home/jupyter-haruki/work/MERRA2/MERRA2_Input_Exp8.nc',decode_times=False)
...
newdata['clwvi'] = xr.DataArray(data['clwvi'].values[:492]*PS_nodiurn.values[:492]/9.81,
                                dims = ["time","lat","lon"],
                                attrs=data['clivi'].attrs)
newdata['crel'] = -1*data['crel'][:492].fillna(0)

for var in ['lcloud','clivi','clwvi','crel']:
    print(var)
...
del data
newdata.to_netcdf('/home/jupyter-haruki/work/MERRA2/MERRA2_Input_Exp8.nc',mode='a')
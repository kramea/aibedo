...
class SphericalConvLSTMUnet(nn.Module):
    ...
    def __init__(self, pooling_class, N, depth, laplacian_type, input_channels, output_channels):
        ...
        self.deconvlstm5 = ConvLSTM(512, 512, 3,  self.laps[1], True, True, False) #in_channel, out_channel should be doubled                                                                                     #as unet gets doubled size of data
    ...
    def forward(self, x):
        ...
        ######DECODER ####################
        #x = self.unpooling(x_enc0)
        #print(x.size(), x_enc1.size())#torch.Size([10240, 162, 1]) torch.Size([10240, 42, 1])
        #exit()
        x = torch.cat((x_enc0, x_enc1), dim=1)
        print(x.size())
        x = self.unpooling(x)

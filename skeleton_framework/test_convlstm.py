...
def main(parser_args):
    ...
    temp_folder="/work/npy_files/" #Change this to where you want .npy files are saved
    ...
    if not parser_args.generation_only:    
        ...
        for i in range(int(len(dataset_tr)/batch_size)):
            images = torch.tensor(dataset_tr[i*batch_size: (i+1)*batch_size]).float().cuda()
            gt_outputs = torch.tensor(dataset_out_tr[i*batch_size: (i+1)*batch_size]).float().cuda()
            print(images.device, gt_outputs.device) #cuda:0 cuda:0
            #images = images.to(device) 
            #gt_outputs = gt_outputs.to(device)
            #print(images.device, gt_outputs.device) #cpu, cpu
            optimizer.zero_grad()
            model.cuda()
            print(images.device, gt_outputs.device) #cuda:0 cuda:0
            outputs = model(torch.tensor(dataset_tr[i*batch_size: (i+1)*batch_size]).float().cuda())
            print(images, gt_outputs)
            
            loss = criterion(outputs.float().cuda(), torch.tensor(dataset_out_tr[i*batch_size: (i+1)*batch_size]).float().cuda())
            exit()
            #loss = criterion(outputs, gt_outputs)
            loss.backward()
            optimizer.step()
            print("Minibatch step "+str(i)+" train loss "+str(loss.item()))
            print("time", time.time() -  time_token)
            time_token =time.time()

...
import torch
from spherical_unet.models.spherical_convlstm.convlstm_model import *
...
def shuffle_data(d1, d2):
    n,_,_,_,_ = np.shape(d1)
    m,_,_,_,_ = np.shape(d2)
    assert(n == m)
...
def main(parser_args):
    ...
    model = ConvLSTM_model( 5, 192, 288 ) #channels, w, d, layers, kernels)
    ...
    print(np.shape(dataset), np.shape(dataset_out)) #(1955, 25, 5, 192, 288) (1955, 25, 3, 1, 288)
    ...
    for epoch in range(0, n_epochs+1):
        ...
        if epoch%1==0:
            if epoch == 0:
                os.mkdir("./saved_model_convlstm_mul/")
            #save model
            torch.save(model.state_dict(), "./saved_model_convlstm_mul/convlstm_state_"+str(epoch)+".pt")
            #test with testset
            with torch.no_grad():
            ...
            np.save("./saved_model_convlstm_mul/prediction_"+str(epoch)+"_"+str(test_loss)+".npy", prediction)
            np.save("./saved_model_convlstm_mul/groundtruth_"+str(epoch)+"_"+str(test_loss)+".npy", groundtruth)

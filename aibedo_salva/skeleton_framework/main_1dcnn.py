...
def normalize(data):
    ...
    for i in range(d1):
        for j in range(d2):
            data_norm[i,j] = (data[i,j]-min_val)/(max_val-min_val)
    return data_norm
...
class CNN(nn.Module):
  def __init__(self):
    ...
    #in_channes = timestep
    self.conv1 = nn.Conv1d(in_channels=3, out_channels=5, kernel_size=5, stride=1, padding=2)
    self.conv2 = nn.Conv1d(in_channels=5, out_channels=10, kernel_size=5, stride=1, padding=2)
    self.conv3 = nn.Conv1d(in_channels=10, out_channels=20, kernel_size=5, stride=1, padding=2)
    self.dropout =  nn.Dropout(p=0.2)
    self.conv4 = nn.Conv1d(in_channels=20, out_channels=1, kernel_size=5, stride=1, padding=2)
...
def main():
    ...
    output_path = "./cnn_"+str(name_of_variable)+"_ts_"+str(input_file_length)+"/"
    n_layers = 7
    hidden_dim = 256
    embedding_dim = 164
    learning_rate = 0.001
